...
namespace RestPOS.Sales_Register
{
    ...
    public partial class SalesRegisterPink : Window
    {
       ...
       // String holdtableno = "";
        ...
        {
            string sql = " select * from tbl_hold_sales_item  group by sales_id, tableno ";
            DataAccess.ExecuteSQL(sql);
            DataTable dt = DataAccess.GetDataTable(sql);
        ...
        private void lstvwHoldList_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            foreach (DataRowView row in lstvwHoldList.SelectedItems)
            {
                ...
                parameter.holdtableno = row.Row[6].ToString();
                lblTableNo.Text = row.Row[6].ToString();
            }
        ...
        public void tablezoneDatabind()
        {
            ...
            string sqltablezone = " select  tz.zonenam, tz.tablename,   tz.fillcolor,  " +
                                 " CASE " +
                                 " WHEN  (cast(strftime('%s', '" + datetime + "')   - (strftime('%s',  tz.ordertime)) AS real) / 60)   < 30  THEN   'B'  " +
                                 "    ELSE '-'  " +
