...
namespace RestPOS.Sales_Register
{
    ...
    public partial class SalesRegister : Window
    {
        ...
        public SalesRegister()
        {
            ...
            /*
            t.Columns.Add(new DataColumn("Artículos"));
            t.Columns.Add(new DataColumn("Precio"));
            t.Columns.Add(new DataColumn("Cant."));
            ...
            t.Columns.Add(new DataColumn("IVA"));
            t.Columns.Add(new DataColumn("KD"));
            t.Columns.Add(new DataColumn("Opciones"));
            */
            ...
            btnholdsale.IsEnabled = true;
            btnReceivePayment.Visibility = Visibility.Visible;
            btnReceivePayment.IsEnabled = true;


            // *** OCULTA BOTONES NO NECESARIOS  ***
            btnholdsale.Visibility = Visibility.Hidden;
            btnDeleteholdsale.Visibility = Visibility.Hidden;
            // btnDirectSale.Visibility = Visibility.Hidden;               // BOTON PAGO DIRECTO SE OCULTA PORQUE TODO SE HACE CON LA VENTANA DE PAGO
            btnReceivePayment.Visibility = Visibility.Hidden;
            btnCompleteSalesAndPrint.Visibility = Visibility.Hidden;    // BOTON COMPLETAR PAGO SE OCULTA PORQUE SE HACE CON EL BOTON PAGO DIRECTO
        ...
        public void tablezoneDatabind()
        {
           string datetime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            ...
            /* QUERY SQLITE ORIGINAL
            string sqltablezone = " select  tz.zonenam, tz.tablename,   tz.fillcolor,  " +
                                 " CASE " +
                                 " WHEN  (cast(strftime('%s', '" + datetime + "')   - (strftime('%s',  tz.ordertime)) AS real) / 60)   < 30  THEN   'R'  " +
                                 "    ELSE '-'  " +
                                 "    END 'booked', tz.seatqty   " +
                                 "  from tbl_tablezone tz       " +
                                 " where tz.status = 1  order by tz.orderno";
         */

            string sqltablezone = " select  tz.zonenam, tz.tablename,   tz.fillcolor,  " +
                                            " CASE " +
                                            " WHEN tz.fillcolor = 'Red'  THEN   'R'  " +
                                            "    ELSE '-'  " +
                                            "    END 'booked', tz.seatqty   " +
                                            "  from tbl_tablezone tz       " +
                                            " where tz.status = 1  order by tz.orderno";
            ...
            //string sqltablezone = " select  tz.zonenam, tz.tablename,   tz.fillcolor,  " +
            //                     " CASE " +
            //                      "  when (cast((time_to_sec(timediff('" + datetime + "', tz.ordertime)) / 60) as decimal(10,0)) < 30) then 'B' " +
            //                       "    ELSE '-'  " +
            //                       "    END 'booked', tz.seatqty   " +
            //              "  from tbl_tablezone tz       " +
            //         "   where tz.status = 1  order by tz.orderno";
        ...
        public void itemslist_withImage(string category)
        {
            ...
            ///// SQLite   -- Enable/uncomment/open below section if you use SQLite Database and Block Mysql section
            string sqlCmd = " Select product_id, product_name, product_quantity , retail_price,   '" + uriSource + "'   || imagename as  imagename " +
                            " from  purchase where " +
                            ...
                            " ORDER BY product_name LIMIT 15 ";  // RANDOM()  Sqlite

            // /// MySQL  -- Enable/uncomment/open below section if you use MySQL Database and Block Above section 
            //string sqlCmd = " Select product_id, product_name, product_quantity , retail_price,   CONCAT('" + uriSource + "', imagename)  as  imagename " +
            //         " from  purchase where " +
            //         " (product_name like '" + category + "%'  and product_quantity >= 1) OR " +
            //         " (product_id like '" + category + "%'  and product_quantity >= 1) OR " +
            //         " (category = '" + category + "' and   product_quantity >= 1) " +
            //         " ORDER BY product_name LIMIT 15 "; // MySQL
        ...
        {
            string result = "OK";
            if (lstvwHoldList.Items.Count > 0)
            {
                MyInputBox inPassword = new MyInputBox();   //ShowDialog means you can't focus the parent window, only the popup
                inPassword.ShowDialog();                    //execution will block here in this method until the popup closes
                result = inPassword.UserPassword;

                if (result != UserInfo.Contrasenia)
                {
                    result = "ERROR";
                    MessageBox.Show("Orden confirmada, solicite autorización del Administrador", "CONTRASEÑA INVALIDA", MessageBoxButton.OK, MessageBoxImage.Warning);
                }
        ...
        {
            string sql = " select * from tbl_hold_sales_item where tableno =  '" + lblTableNo.Text + "'  group by sales_id, tableno";
            DataAccess.ExecuteSQL(sql);
            DataTable dt = DataAccess.GetDataTable(sql);
        ...
        private void btnholdsale_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
                growlNotifications.AddNotification(new Notification { Title = "Alert Message", Message = "Agregue Productos a la Orden", ImageUrl = "pack://application:,,,/Notifications/Radiation_warning_symbol.png" });
            }
            else
            {
                try
                {                                           // Guarda articulos de la orden
                    Hold_sales_item(DateTime.Now.ToString("yyyy-MM-dd").ToString());

                    tokennoInsert();                        // Guarda Id de Venta y Token
                    tokennumberget();                       // Incrementa Token si orden es de la misma fecha
                    // // Change Hold Status to normal
                    if (parameter.resumesalesstatus == "1") { resumestatuschange(parameter.holdtransactionID); }
                    HolditemDatabind();                     // Llena vista con artículos de la orden reservada
                    parameter.resumesalesstatus = "2";

                    updatetablebooked();                    // Reserva Mesa y la pone en rojo en lista de mesas  
                   
                    Sales_Register.PrintTicket go = new Sales_Register.PrintTicket();
                    go.Show();
		            tabstockcontrol.SelectedItem    = tabholdtransactions;
                    // btnholdsale.Visibility          = Visibility.Hidden;     // BOTON RESERVA MESA
                    //btnDeleteholdsale.Visibility = Visibility.Hidden; 
                    // tabPayment.Visibility           = Visibility.Hidden;     // PESTAÑA PAGAR

                    //btnDirectSale.IsEnabled     = true;                         // BOTON PAGAR CON EFECTIVO
                    //btnReceivePayment.IsEnabled = true;                         // BOTON PAGAR
                    ...
                    //txtbarcodescan.Focus();
                    tablezoneDatabind();
                    tabSalesRegister.SelectedItem = tabtableLayout;
        ...
        public void updatetablefree()  // Libera orden y pone color en verde
        {
            string orderdatetime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            string sqlUpdateCmd = " update tbl_tablezone set fillcolor='Green', ordertime = '' where tablename = '" + lblTableNo.Text + "'";
            DataAccess.ExecuteSQL(sqlUpdateCmd); 
        }

        public void updatetablebooked()  // Reserva orden y pone color en rojo
        {
            string orderdatetime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
            string sqlUpdateCmd = " update tbl_tablezone set fillcolor='Red', ordertime = '" + orderdatetime + "'    where tablename = '" + lblTableNo.Text + "'";
            DataAccess.ExecuteSQL(sqlUpdateCmd);
        }
        ...
        private void btnDirectSale_Click(object sender, RoutedEventArgs e)
        {
            ...
            {
               MessageBox.Show("Su licencia ha alcanzado el limíte de transacciones \n contactar al desarrollador: pedro.rubio@vital-software.com \nWhatsApp:55-5455-4879\nhttp://www.vital-software.com", "Yes or No", MessageBoxButton.OK, MessageBoxImage.Warning);
            }        
            else
            {
                try
                {
                    ////Save payment info into sales_payment table  *** ENCABEZADO ORDENES PAGADAS ***
                    payment_item(lblTotalPayable.Text, "0", "0", "Efectivo", DateTime.Now.ToString("yyyy-MM-dd").ToString(), "10000028", "Guest");

                    ///// save sales items one by one               *** DETALLE PLATILLOS ORDENES PAGADAS ***
                    ...
                    //// Booked Table                           
                    // updatetablebooked();                         *** LIBERA MESA RESERVADA ***
                    updatetablefree();                              
                    ...
                    lstvwHoldList.SelectedItems.Clear();        // RUSP 23/OCT/2020 : LIMPIA LISTA MESAS RESERVADAS *** PROBAR
                    tabPayment.Visibility = Visibility.Hidden;
                    t.Rows.Clear();
                    DiscountCalculation();
                    vatcal();
        ...
        private void btnSuspend_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                btnholdsale.Visibility = Visibility.Hidden;
                //btnDirectSale.Visibility = Visibility.Hidden;
                btnReceivePayment.Visibility = Visibility.Hidden;
                btnDeleteholdsale.Visibility = Visibility.Hidden; 
            }
            catch (Exception exp)
        ...
        private void dgrvSalesItemList_CellEditEnding(object sender, DataGridCellEditEndingEventArgs e)
        {
            try
            {
                ...
                for (i = 0; i < dgrvSalesItemList.SelectedItems.Count; i++)
                {
                    ...
                    //if (e.Column.SortMemberPath.Equals("Cant."))
                    {
        ...
        {            
            ...
            //txtCustName.SelectedIndex.Equals(-1);
            //txtCustName.SelectedItem = 0;
            txtCustName.SelectedIndex = txtCustName.SelectedIndex + 1;
        }
        ...
        //Sale and Direct print
        private void btnCompleteSalesAndDirectPrint_Click(object sender, RoutedEventArgs e)
        {
            if (txtPaidAmount.Text == "00" || txtPaidAmount.Text == "0" || txtPaidAmount.Text == string.Empty)
            {
                //  MessageBox.Show("Please insert paid amount", "Yes or No", MessageBoxButton.OK, MessageBoxImage.Warning);
                growlNotifications.AddNotification(new Notification { Title = "Alert Message", Message = "Incluye el monto pagado", ImageUrl = "pack://application:,,,/Notifications/Radiation_warning_symbol.png" });
                txtPaidAmount.Focus();
            }
            else if ( txtCustName.SelectedIndex.Equals(-1) )
            {
                MessageBox.Show("Seleccion un Cliente de la lista", "Yes or No", MessageBoxButton.OK, MessageBoxImage.Warning);
                // if (Convert.ToInt64(txtInvoice.Text) >= InvoicesManager.InvoiceNo)
            ...
            else
            {
                try
                {
                    if (MessageBox.Show("Realmente desea completar está Orden?", "Cerrar Orden", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                    {
                        ////Save payment info into sales_payment table
                        payment_item(lblTotalPayable.Text, txtChangeAmount.Text, txtDueAmount.Text, CombPayby.Text, dtSalesDate.Text, lblCustID.Text, txtcomment.Text);
                        ...
                        */
                        ...
                        lstvwHoldList.SelectedItems.Clear();        // RUSP 23/OCT/2020   LIMPIA LISTA DE MESAS RESERVADAS   *** PROBAR
                        tabPayment.Visibility = Visibility.Hidden;
                        t.Rows.Clear();
                        DiscountCalculation();
                        vatcal();
        ...
        private void txtPaidAmount_TextChanged(object sender, TextChangedEventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    btnCompleteSalesAndDirectPrint.Focus();
                }
                catch //(Exception exp)
                {
        ...
        */
        ...
        private void PrintReceiptWithoutPrintDialog()
        {
            ...
            // 🔹 DEFINIR IMPRESORA DIRECTA
            printDocument.PrinterSettings.PrinterName = "ImpresoraCaja";

            // 🔹 validar que exista
            if (!printDocument.PrinterSettings.IsValid)
            {
                MessageBox.Show("La impresora 'ImpresoraCaja' no está instalada.");
                return;
            }
        ...
        void printDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            string sql = " SELECT  sp.sales_id AS salesid, sp.payment_type AS paytype, sp.payment_amount AS Payamount, " +
                            ...
                            " sp.ordertime AS ordertime, sp.tableno AS tableno, sp.tokenno AS tokenno, sp.Shopid," +
                            " sp.c_id AS custID, sp.emp_id AS empID, sp.comment AS Note, sp.TrxType, si.sales_id, si.item_id,  " +
                            " si.itemName, si.Qty, si.RetailsPrice, si.Total,si.profit, si.sales_time, " +
                            " si.note AS notesi, sp.vaterate as varate,  tl.*, c.*,  " +
                            " CASE     " +
            ...
            string storename = dt.Rows[0]["companyname"].ToString();    //"Cafeína Coffee Shop"
            string Address = dt.Rows[0]["location"].ToString();         // "Av. Cuautemoc No. 1550, Ixtapaluca"
            string Phone = dt.Rows[0]["phone"].ToString();              // "+1(52)"
            string vatregino = dt.Rows[0]["vatregino"].ToString();      // "RFC"
            string Salesid = "# ORDEN: " + dt.Rows[0]["salesid"].ToString() + "  " + dt.Rows[0]["empID"].ToString();
            string tableno = dt.Rows[0]["tableno"].ToString() + "  Token: " + dt.Rows[0]["tokenno"].ToString();
            string currencysign = dt.Rows[0]["currencysign"].ToString();

            offset = offset + (int)fontHeight;
            RectangleF rectFStorename = new RectangleF(startX, offset, 245, 120);
            graphic.DrawString(storename, new Font("Courier New", 14), new SolidBrush(System.Drawing.Color.Black), rectFStorename);

            offset = offset + (int)fontHeight + 23;
            RectangleF rectFaddr = new RectangleF(startX, offset, 180, 55);
            graphic.DrawString("".PadRight(3) + Address, font, new SolidBrush(System.Drawing.Color.Black), rectFaddr);

            //offset = offset + (int)fontHeight + 7;  // +Convert.ToInt32(rectFaddr.Height);
            //graphic.DrawString("".PadRight(3) + "TEL:" + Phone, font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);

            offset = offset + (int)fontHeight + 5;
            graphic.DrawString(DateTime.Now.ToString(), font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);

            offset = offset + (int)fontHeight + 3;
            graphic.DrawString(Salesid, font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);

            offset = offset + (int)fontHeight + 3;
            graphic.DrawString(tableno, font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                string TaxApply = dt.Rows[i]["TaxApply"].ToString();
                string sauce = dt.Rows[i]["notesi"].ToString();
                string strLinea = productdescription;
                strLinea = strLinea.Trim();



                if (strLinea.Length >= 20)
                {
                     product1Line = " " + strLinea.Substring(0, 20) + "  " + productQty + " X " + currencysign + productTotal; // + " " + TaxApply + " " + sauce;
                }
                else
                {
                     product1Line = " " + strLinea + "  " + productQty + " X " + currencysign + productTotal; // + " " + TaxApply + " " + sauce;
                }
                
                RectangleF rectProdline = new RectangleF(startX, offset, 315, 85);
                graphic.DrawString(product1Line, font, new SolidBrush(System.Drawing.Color.Black), rectProdline);
                offset = offset + (int)fontHeight + 10; // 13;
            }

            decimal dis = Convert.ToDecimal(dt.Rows[0]["dis"].ToString());
            decimal TAX = Convert.ToDecimal(dt.Rows[0]["vat"].ToString());
            string TAXrate = dt.Rows[0]["varate"].ToString() + "%";
            ...
            offset = offset + (int)fontHeight + 5;
            graphic.DrawString("Sub-Total ".PadRight(15) + currencysign + Subtotal.ToString(), font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);

            offset = offset + (int)fontHeight + 5;
            graphic.DrawString("IVA " + TAXrate.ToString().PadRight(11) + currencysign + TAX.ToString(), font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);

            offset = offset + (int)fontHeight + 15;
            RectangleF rectTotal = new RectangleF(startX, offset, 215, 127);
            graphic.DrawString("Total ".PadRight(8) + currencysign + payment_amount, new Font("Courier New", 14, System.Drawing.FontStyle.Bold), new SolidBrush(System.Drawing.Color.Black), rectTotal);

            if (Change > 0)
            {
                offset = offset + (int)fontHeight + 17;
                graphic.DrawString("Cambio  ".PadRight(15) + currencysign + Change.ToString(), font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);
            }

            if (due > 0)
            {
                offset = offset + (int)fontHeight + 17;
                graphic.DrawString("Due  ".PadRight(15) + currencysign + due.ToString(), font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);
            }

            if (dis > 0)
            {
                offset = offset + (int)fontHeight + 17;
                graphic.DrawString("Discount ".PadRight(15) + currencysign + dis.ToString(), font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);
            }

            offset = offset + (int)fontHeight + 5;
            graphic.DrawString("Forma Pago ".PadRight(15) + String.Format("{0:N}", Payment), font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);


            offset = offset + (int)fontHeight + 13;
            RectangleF rectF1 = new RectangleF(startX, startY + offset, 210, 106);
            graphic.DrawString(footermsg, font, new SolidBrush(System.Drawing.Color.Black), rectF1);

            // Salto de linea obligado
            offset = offset + (int)fontHeight + 5;
            graphic.DrawString(".+.+.+.+.+.+.+.+.+.+.+.+.+.+.+", font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);
            offset = offset + (int)fontHeight + 5;
            graphic.DrawString(".+.+.+.+.+.+.+.+.+.+.+.+.+.+.+", font, new SolidBrush(System.Drawing.Color.Black), startX, startY + offset);
        ...
        private void DgrvSalesItemList_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
